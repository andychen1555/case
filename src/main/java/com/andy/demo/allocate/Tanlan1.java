...
//        List<Integer> list1 = Arrays.asList(1832, 1640, 2725, 1710, 2213, 3111, 2328, 5589, 3386, 2331, 2868);
        List<Integer> list = Arrays.asList(1, 2, 3, 6, 8, 10, 18);
...
//        Integer max = new StandrdDeviation1().getStandDeviation1(list);
...
//                filterList.size(), n, filterList.stream().mapToInt(Integer::valueOf).sum(), filterList.stream().mapToInt(Integer::valueOf).sum() / n);
        new TanlanA1().Distrube(list, n);

    }
}

class StandrdDeviation1 {
    // 求标准差
